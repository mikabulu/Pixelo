<template>
    <div class="p-4">
      <div class="columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
        <div
          v-for="post in postsWithAttachments"
          :key="post.id"
          class="break-inside-avoid rounded-lg overflow-hidden mb-4"
        >
          <RouterLink :to="{ name: 'postview', params: { id: post.id } }">
            <img
              v-if="post.attachments[0]?.get_image"
              :src="post.attachments[0].get_image"
              class="w-full h-auto block"
              alt="Post image"
            />
            <video
              v-else-if="post.attachments[0]?.get_video"
              controls
              class="w-full h-auto block"
            >
